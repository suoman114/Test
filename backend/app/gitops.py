...
def _run(args: list[str], cwd: str | None, token: str) -> subprocess.CompletedProcess:
    """git 명령을 실행한다.

    토큰을 Authorization 헤더로 주입하고, 실패 시 stderr 를 포함한 예외를 던진다.
    토큰은 URL 이 아닌 헤더로만 들어가므로 process 인자에 평문 노출되지 않는다.
    """
...
def publish_version(
    ...
    try:
        ...
        _run(["config", "user.name", name], cwd=tmpdir, token=token)
        _run(["config", "user.email", email], cwd=tmpdir, token=token)
        ...
        # 3. add + commit
        _run(["add", settings.package_tar_path], cwd=tmpdir, token=token)
        _run(["commit", "-m", commit_message], cwd=tmpdir, token=token)
        ...
        # 5. push branch + tag
        _run(["push", "origin", branch], cwd=tmpdir, token=token)
        _run(["push", "origin", version], cwd=tmpdir, token=token)
